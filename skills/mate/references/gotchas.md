...
`--force-keep-alive` requires the `bin/mate` wrapper (not raw `bin/mate.php`) : the wrapper loops the child on non-zero exits and exits only on signals (`bin/mate` lines 26-62).
...
## 19. `mate/.env` is gitignored and not auto-loaded

`resources/mate/.gitignore` contains exactly one line: `.env.local`. So the file `mate/.env` itself is committed (empty by default), but `mate/.env.local` is not. `mate/.env` is also not loaded unless you set `mate.env_file` (parameter) in `mate/config.php` AND have `symfony/dotenv` installed : otherwise loading is a no-op.
