...
`--force-keep-alive` makes `bin/mate` (the wrapper) loop the child process on non-zero exits so editors auto-restart the server. Without it the server exits when the agent changes configuration and you must `mate stop` first.
