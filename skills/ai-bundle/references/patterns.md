...
Then run `bin/console ai:store:index ai.indexer.docs` to populate the store (the `ai.command.index` service is wired in `config/services.php` lines 312-316).
...
ai:
    ...
    chat:
        support:
            ...
            message_store: 'ai.message_store.doctrine.support'
```
