...
Defined in `config/platform/*.php`. Each provider key is a top-level child of `ai.platform`. Some providers are singleton (`openai`, `anthropic`, `ollama`, `gemini`, `minimax`, `mistral`, `cerebras`, `cohere`, `deepseek`, `voyage`, `perplexity`, `openrouter`, `huggingface`, `vertexai`, `transformersphp`, `cartesia`, `decart`, `elevenlabs`, `deepgram`, `lmstudio`, `dockermodelrunner`, `albert`, `amazeeai`); others take named instances (`azure`, `bedrock`, `cache`, `failover`, `generic`, `openresponses`).
    ...
    platform:
        ...
        failover:
            primary:
                ...
                rate_limiter: 'limiter.openai'   # required service id
...
- **`max_tool_calls`, `circuit_breaker_threshold`, etc. are NOT valid keys** : only `fault_tolerant_toolbox: bool` exists for fault tolerance.
...
ai:
    message_store:
        ...
        redis:
            support:
                client: null                # service id (or null => build from connection_parameters)
