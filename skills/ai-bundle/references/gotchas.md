...
`ProcessorCompilerPass::process()` lines 64-66 sort each agent's input and output processors by priority descending (higher = first input, last output). If you register the same processor via attribute **and** manually in `services.yaml` with two different priorities, you get two tags on the same service and the order is non-deterministic. Stick to one registration path.
