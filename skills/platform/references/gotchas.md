...
- `Image::fromBinary()` does **not** exist. Build the `Image` (or any
  ...
  : the second parameter is `$format` (e.g. `'image/png'`), not a MIME type name.
- Provider size limits differ: OpenAI 20 MB images, Anthropic 5 MB,
  Gemini 20 MB. Exceeding them throws `InvalidArgumentException` upstream
  in the bridge.
...
| `ModelNotFoundException`        | extends `\InvalidArgumentException` (no `ExceptionInterface`!) |
...
`Factory::create(null)` (no key) throws. The factories only accept
non-empty API key strings. If you need lazy key resolution, defer
construction until first call (Symfony service factories support this)
or use a service that wraps the factory call.
...
- Metadata is **not** complete before the stream is exhausted.
  `MetaDataStreamListener` and `TokenUsageStreamListener` push deltas as
