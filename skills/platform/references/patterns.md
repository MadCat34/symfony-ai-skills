...
do {
    ...
    if ($reason?->is(FinishReasonCase::TOOL_CALL)) {
        ...
        foreach ($calls as $call) {
            // Resolve the method on the reference class
            $ref = $tool->getReference();
            $args = $call->getArguments();
            $output = (new $ref->getClass())->{$ref->getMethod()}($args);

            $bag = $bag->with(Message::ofToolCall(
                $call,
                Message::ofUser($output),
            ));
        }
...
`Symfony\AI\Platform\Bridge\Failover\FailoverPlatform` (inside the
`symfony/ai-platform` package, under `Bridge/Failover/`). Its constructor
requires a `RateLimiterFactoryInterface` as the second argument.
...
- Metadata (`token_usage`, `finish_reason`, provider-specific fields) is only
  ...
  `MetaDataStreamListener` and `TokenUsageStreamListener` push deltas as they
  arrive.
...
inside the `symfony/ai-platform` package, under `Bridge/Failover/`) is a
...
- forwards the **same `$model` argument** (string or `Model`) to every
  underlying platform;
- consults a `RateLimiterFactoryInterface` per platform to short-circuit
  saturated ones (the limiter is also consumed on a thrown call, so the
  failure still costs a token);
- tracks failed platforms in a `\WeakMap` keyed by platform instance and
  unmarks them once the rate limiter accepts them again;
