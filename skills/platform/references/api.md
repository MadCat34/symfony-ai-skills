...
├── Exception\                      (20 exceptions — see gotchas.md)
...
The second argument is intentionally permissive. `StringToMessageBagListener`
auto-wraps a bare `string` into a `MessageBag` containing a single `UserMessage`,
and `Contract` normalises `MessageBag`, single messages, raw arrays, and
serializable objects alike.
...
| `ThinkingResult`       | `string` (text content only)                 |
