...
# Pick a bridge for persistence. The package is named after the backend, not
# after Chat: symfony/ai-<backend>-message-store.
...
$store = new Store(); // InMemory\Store: implements both ManagedStoreInterface and MessageStoreInterface
...
foreach ($chat->stream(Message::ofUser('Tell me a joke.')) as $delta) {
    // DeltaInterface is an empty marker — filter for TextDelta before printing.
