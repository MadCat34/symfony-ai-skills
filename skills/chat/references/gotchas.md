...
$store = new Store(); // InMemory\Store, or any bridge, keyed by constructor ($key, $cacheKey, $indexName, $sessionKey)
...
The constructor signature is `(string $tableName, Connection $dbalConnection, SerializerInterface $serializer = new Serializer(...), ClockInterface $clock = new MonotonicClock())`. Neither `$serializer` nor `$clock` is nullable — they have concrete defaults (`Serializer` with standard normalizers and `JsonEncoder`, plus `MonotonicClock`). Passing `null` to either raises a `TypeError`. Table name is **first** and does not have a default : you must pass it. See `api.md` for the exact default object signatures.
...
```bash
# Composer
composer require symfony/ai-meilisearch-message-store symfony/clock
```
