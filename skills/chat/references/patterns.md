...
```bash
php bin/console ai:message-store:setup chat.store.doctrine
```
...
class Weather
{
    #[AsTool('Get current weather for a city.', 'get_weather')]
...
$platform = OpenAiFactory::createPlatform($_ENV['OPENAI_API_KEY']);
$agent = new Agent($platform, 'gpt-4o-mini', [new Toolbox(new Weather())]);

$chat = new Chat($agent, new InMemory\Store());
...
$inner = new Chat($agent, new InMemory\Store());
...
$store = new TraceableMessageStore(new InMemory\Store(), new MonotonicClock());
$chat = new Chat($agent, $store);
...
- `skills/recipes/chat-with-memory-doctrine.md` : end-to-end recipe using persistent message storage.
